    ...
    try {
        ...
        if (isPlaylist) {
            ...
            for (const lock of lockups) {
                ...
                let thumb = [];
                if (sources.length > 0) {
                    thumb = [{ url: sources[sources.length - 1].url.split('?')[0] }];
                }
            ...
            for (const item of items) {
                if (item.musicResponsiveListItemRenderer) {
                    ...
                    let thumb = [];
                    if (i.thumbnail?.musicThumbnailRenderer?.thumbnail?.thumbnails) {
                        thumb = i.thumbnail.musicThumbnailRenderer.thumbnail.thumbnails;
                    }
