const App={
    init(){
        ...
        <div class="pt-12 px-4 text-center">
            ...
            <h1 class="text-3xl font-black text-[#1ed760] mb-1">HanzMusify</h1>
            ...
            <div class="bg-[#181818] rounded-2xl p-5 max-w-sm mx-auto border border-white/10 space-y-3 text-left mb-4">
                ...
                <div class="flex justify-between"><span class="text-[#6b7280] text-sm">Nama</span><span class="text-white font-medium text-sm">HanzMusify</span></div>
    ...
    autoPlayTrack(videoId){
        ...
        .then(function(d){
            var title='Lagu',artist='HanzMusify',cover=FI,artistId='';
        ...
        .catch(function(){
            S.ct={id:videoId,videoId:videoId,title:'Lagu',artist:'HanzMusify',cover:FI,artistId:'',ytUrl:'https://youtube.com/watch?v='+videoId};
            S.ps='direct';
            S.pl=[S.ct];
            S.pi=0;
            S.lyricsLoaded=false;
    ...
    showSharePopup(videoId){
        ...
        .then(function(r){return r.json();})
        .then(function(d){
            var title='Lagu',artist='HanzMusify',cover=FI;
            if(d.status && d.result.songs && d.result.songs.length>0){
                var song=d.result.songs[0];
                title=cn(song.title);
                artist=cn(song.artist);
        ...
        .catch(function(){
            App.renderPopup(videoId,'Lagu','HanzMusify',FI);
        });
    },
...
var currentLibraryTab = 'playlist';

function switchLibraryTab(tab){
    currentLibraryTab = tab;
    document.querySelectorAll('.lib-tab').forEach(function(el){
        el.classList.remove('active','bg-[#1ed760]','text-black');
        el.classList.add('text-[#6b7280]');
    });
    var btn = gid('lib-tab-'+tab);
    if(btn){
        btn.classList.add('active','bg-[#1ed760]','text-black');
        btn.classList.remove('text-[#6b7280]');
    }
    Library.render();
}

function toggleLikeTrack(videoId){
    var likes = getLikedSongs();
    var idx = likes.findIndex(function(s){return s.videoId===videoId;});
    if(idx>-1){
        likes.splice(idx,1);
        ...
        if(S.ct){
            likes.push({
                ...
                artist:S.ct.artist,
                cover:S.ct.cover,
                artistId:S.ct.artistId||'',
                ytUrl:S.ct.ytUrl
            });
            showToast('Ditambahkan ke Disukai');
        }
    }
    saveLikedSongs(likes);
    updateLikeButton();
    if(currentLibraryTab==='liked') Library.render();
}

function getLikedSongs(){
    try{return JSON.parse(localStorage.getItem('hanzz_liked_songs')||'[]');}
    catch(e){return [];}
}

function saveLikedSongs(songs){
    try{localStorage.setItem('hanzz_liked_songs',JSON.stringify(songs));}
    catch(e){}
}

function isLiked(videoId){
    var likes = getLikedSongs();
    return likes.some(function(s){return s.videoId===videoId;});
}

function updateLikeButton(){
    if(!S.ct) return;
    var liked = isLiked(S.ct.videoId);
    var btns = document.querySelectorAll('.like-btn');
    btns.forEach(function(btn){
        if(liked){
            btn.innerHTML = '<i data-lucide="heart" class="w-5 h-5 fill-[#1ed760] text-[#1ed760]"></i>';
        }else{
...
const Library={
    render(){
        ...
        html+='<button id="lib-tab-playlist" onclick="switchLibraryTab(\'playlist\')" class="lib-tab px-4 py-2 rounded-full text-sm font-medium transition-all bg-[#1ed760] text-black">Playlist</button>';
        html+='<button id="lib-tab-liked" onclick="switchLibraryTab(\'liked\')" class="lib-tab px-4 py-2 rounded-full text-sm font-medium transition-all text-[#6b7280]">Disukai</button>';
        html+='<button id="lib-tab-artist" onclick="switchLibraryTab(\'artist\')" class="lib-tab px-4 py-2 rounded-full text-sm font-medium transition-all text-[#6b7280]">Artist</button>';
        html+='</div>';
        
        if(currentLibraryTab==='liked'){
            ...
            if(likes.length===0){
                html+='<div class="text-center text-[#6b7280] mt-10"><i data-lucide="heart" class="w-16 h-16 mx-auto mb-4 opacity-30"></i><p>Belum ada lagu disukai</p></div>';
            }else{
                html+='<div class="space-y-1">';
                likes.forEach(function(s,i){
                    html+='<div onclick="Library.playLiked('+i+')" class="flex items-center gap-3 p-3 hover:bg-[#282828] rounded-lg cursor-pointer active:scale-[0.98] transition-colors"><img src="'+getHDImage(s.cover)+'" class="w-10 h-10 rounded object-cover" /><div class="truncate"><p class="font-medium text-sm truncate">'+s.title+'</p><p class="text-[#6b7280] text-xs truncate">'+s.artist+'</p></div><button onclick="event.stopPropagation();toggleLikeTrack(\''+s.videoId+'\')" class="text-[#1ed760] p-2"><i data-lucide="heart" class="w-4 h-4 fill-[#1ed760] text-[#1ed760]"></i></button></div>';
                });
                html+='</div>';
            }
        }else if(currentLibraryTab==='artist'){
            html+='<button onclick="Library.createNew()" class="w-full bg-[#1ed760] text-black font-bold py-3 rounded-xl active:scale-95 mb-4">+ Buat Playlist Baru</button>';
            html+='<div id="artist-list-content" class="text-center text-[#6b7280] mt-10">Fitur Artist akan ditampilkan di sini</div>';
        }else{
            html+='<button onclick="Library.createNew()" class="w-full bg-[#1ed760] text-black font-bold py-3 rounded-xl active:scale-95 mb-4">+ Buat Playlist Baru</button>';
            if(pls.length===0){
    ...
    open(id){
        ...
        if(pl.songs.length===0){
            ...
            pl.songs.forEach(function(s,i){
                var liked = isLiked(s.videoId);
                html+='<div class="flex items-center gap-3 p-3 hover:bg-[#282828] rounded-lg transition-colors"><div onclick="Library.playSong(\''+id+'\','+i+')" class="flex items-center gap-3 flex-1 cursor-pointer"><img src="'+getHDImage(s.cover)+'" class="w-10 h-10 rounded object-cover" /><div class="truncate"><p class="font-medium text-sm truncate">'+s.title+'</p><p class="text-[#6b7280] text-xs truncate">'+s.artist+'</p></div></div><button onclick="event.stopPropagation();toggleLikeTrack(\''+s.videoId+'\')" class="text-[#6b7280] hover:text-[#1ed760] p-2 transition-colors">'+(liked?'<i data-lucide="heart" class="w-4 h-4 fill-[#1ed760] text-[#1ed760]"></i>':'<i data-lucide="heart" class="w-4 h-4"></i>')+'</button></div>';
            });
            html+='</div>';
        }
        html+='</div>';
