const MP={
    init(){
        ...
        <div id="mini-player" class="hidden fixed left-3 right-3 z-50" style="bottom:80px;transition:transform 0.35s ease-out;transform:translateY(40px);">
            <div class="rounded-2xl p-3 flex items-center gap-3 cursor-pointer active:scale-[0.98] transition relative overflow-hidden" style="background:rgba(24,24,24,0.95);backdrop-filter:blur(20px);border:1px solid rgba(255,255,255,0.1);box-shadow:0 10px 40px rgba(0,0,0,0.6);">
                <div class="absolute top-0 left-0 h-[3px] bg-white/10 w-full"><div id="mini-progress" class="h-full bg-gradient-to-r from-[#1ed760] to-[#1fdf64]" style="width:0%"></div></div>
                ...
                <button onclick="event.stopPropagation();toggleLikeTrack()" class="like-btn text-[#b3b3b3] hover:text-[#1ed760] active:scale-90 p-2 transition-colors"><i data-lucide="heart" class="w-4 h-4"></i></button>
        ...
        <div id="mini-sidebar" class="hidden fixed right-0 z-50" style="bottom:80px;transition:all 0.3s ease-out;">
            <button onclick="MP.restore()" class="bg-[#1ed760] text-black rounded-l-full p-3 shadow-lg active:scale-95 transition-all">
